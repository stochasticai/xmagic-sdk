...
Much of this codebase is written and maintained through
[Claude Code](https://docs.anthropic.com/en/docs/claude-code). The repo carries
its configuration under `.claude/` so the same rules apply to every session,
not just the sessions of whoever set them up. Nothing there is needed to build
or test the package.

- **`.claude/settings.json`** turns off AI attribution in commits and pull
  requests at the source, registers the hooks below, and allowlists a handful
  of read-only commands (`git fetch`, `gh run watch`, the ruff, mypy, and
  pytest checks) so they run without a prompt. Personal overrides go in
  `.claude/settings.local.json`, which is gitignored.
- **`.claude/hooks/`** are enforced by the harness, whatever the model
  intends. `check-attribution.sh` refuses a `git commit`, `gh pr`, `gh issue`,
  or `gh release` command whose text carries attribution, using the same
  `scripts/check_attribution.py` the git hook and CI use. `block-push-main.sh`
  refuses any `git push` that would land on `main`; tags and branches pass.
  `ruff-format.sh` formats a `.py` or `.md` file right after the agent writes
  it.
- **`.claude/skills/`** are procedures the agent follows on request. Each one
  points at the human document rather than repeating it: `release` follows
  [RELEASING.md](RELEASING.md), `record-fixture` follows
  [tests/fixtures/README.md](tests/fixtures/README.md). Change the document,
  not the skill.

There is deliberately no `CLAUDE.md`. Anything an agent needs to know belongs
in the same documents a human contributor reads: this file, RELEASING.md,
DESIGN.md, and the README under `tests/fixtures/`.
